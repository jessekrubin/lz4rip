...
#[derive(Default)]
pub enum BlockSize {
    /// Detect optimal block size from the first write. Resolved automatically
    /// by [`super::FrameEncoder`] before the first block is written.
    #[default]
    Auto = 0,
...
#[derive(Clone, Copy, PartialEq, Eq, Debug)]
/// Block dependency mode for frame compression.
#[derive(Default)]
pub enum BlockMode {
...
pub struct FrameInfo {
    ...
    /// Set via [`super::FrameEncoder::with_dictionary`] / [`super::FrameDecoder::with_dictionary`].
...
impl FrameInfo {
    ...
    pub(crate) fn write(&self, output: &mut [u8]) -> Result<usize, Error> {
        ...
        let bs = self.block_size as u8;
        if !(4..=7).contains(&bs) {
            return Err(Error::UnsupportedBlocksize(bs));
        }
        buffer[5] = bs << BD_BLOCK_SIZE_MASK_RSHIFT;
